import sys

sys.path.append("../sun/src/")
import import_utils
import toolbox as tb
import numpy as np
import pandas as pd
import utils
import matplotlib.pyplot as plt
from glob import glob as glob
from natsort import natsorted as ns
from Response import Response as Res
from SegmentationMap import SegmentationMap as SM
import analysis


def evidence_integration_df_from_files(
    R, decision_bounds=None, n_trials=10, n_pseudocoords=10, use_pointwise_rt=None
):

    R.get_np_coords()
    points = R.points
    pairs = R.get_tested_pairs(all_pairs=False)

    if decision_bounds is not None:
        pass
    else:
        decision_bounds = {"const": [0.6, -0.6]}

    KMeans = SM((0, R.image), mode="array")

    KMeans.fit_model(
        model="c",
        n_components=np.array([R.kSeg]),
        layer_stop=1,
        keep=True,
        init=None,
        spatial_smoothing=1,
    )
    df_kmeans = KMeans.get_decision_rts(
        points,
        pairs,
        R.testedPairs,
        use_pointwise_rts=None,
        use_pseudocoords=n_pseudocoords,
        boundary=decision_bounds,
        mode="ei",
    )
    df_kmeans["condition"] = "kmeans"
    # df_kmeans["smooth"] = 1

    # KMeans_unsmooth.fit_model(
    # model="c",
    # n_components=np.array([R.kSeg]),
    # layer_stop=1,
    # keep=True,
    # init=None,
    # spatial_smoothing=0,
    # )
    # df_kmeans_unsmooth = KMeans_unsmooth.get_decision_rts(
    # points,
    # pairs,
    # R.testedPairs,
    # use_pointwise_rts=None,
    # use_pseudocoords=n_pseudocoords,
    # boundary=decision_bounds,
    # mode="ei",
    # )
    # df_kmeans_unsmooth["condition"] = "kmeans"
    # df_kmeans_unsmooth["smooth"] = 0

    # const_test = [1.2, 2.4, 4.8]

    # dfs_kmeans_test = pd.concat(
    # [KMeans.reapply_bounds({"const": [b, -b]}, key=b) for b in const_test],
    # ignore_index=True,
    # )

    # dfs_kmeans_test_unsmooth = pd.concat(
    # [KMeans_unsmooth.reapply_bounds({"const": [b, -b]}, key=b) for b in const_test],
    # ignore_index=True,
    # )

    # df_kmeans_all = pd.concat(
    # [df_kmeans, dfs_kmeans_test, df_kmeans_unsmooth, dfs_kmeans_test_unsmooth]
    # )

    # df_kmeans_all["trial"] = -1

    # df_trials = []

    # for i in range(n_trials):
    # voronoi_prior = utils.generate_random_voronoi_prior(
    # *R.image.shape[:2], R.kSeg, end_iter=0.05
    # )
    # random_prior = utils.generate_random_prior(*R.image.shape[:2], R.kSeg)

    # Voronoi = SM((0, R.image), mode="array")
    # Random = SM((1, R.image), mode="array")
    # Unsmooth = SM((2, R.image), mode="array")

    # Voronoi.fit_model(
    # model="c",
    # n_components=np.array([R.kSeg]),
    # layer_stop=1,
    # keep=True,
    # init=voronoi_prior,
    # spatial_smoothing=1,
    # init_eps=0.05,
    # )
    # Random.fit_model(
    # model="c",
    # n_components=np.array([R.kSeg]),
    # layer_stop=1,
    # keep=True,
    # init=random_prior,
    # spatial_smoothing=1,
    # init_eps=0.1,
    # )
    # Unsmooth.fit_model(
    # model="c",
    # n_components=np.array([R.kSeg]),
    # layer_stop=1,
    # keep=True,
    # init=voronoi_prior,
    # spatial_smoothing=0,
    # init_eps=0.05,
    # )

    # df_voronoi = Voronoi.get_decision_rts(
    # points,
    # pairs,
    # R.testedPairs,
    # use_pointwise_rts=None,
    # use_pseudocoords=n_pseudocoords,
    # boundary=decision_bounds,
    # mode="ei",
    # )
    # df_voronoi["condition"] = "voronoi"
    # df_voronoi["smooth"] = 1

    # const_test = [0.6, 1.2, 2.4, 4.8]

    # dfs_test = pd.concat(
    # [Voronoi.reapply_bounds({"const": [b, -b]}, key=b) for b in const_test],
    # ignore_index=True,
    # )

    # df_random = Voronoi.get_decision_rts(
    # points,
    # pairs,
    # R.testedPairs,
    # use_pointwise_rts=None,
    # use_pseudocoords=n_pseudocoords,
    # boundary=decision_bounds,
    # mode="ei",
    # )
    # df_random["condition"] = "random"
    # df_random["smooth"] = 1

    # df_unsmooth = Unsmooth.get_decision_rts(
    # points,
    # pairs,
    # R.testedPairs,
    # use_pointwise_rts=None,
    # use_pseudocoords=n_pseudocoords,
    # boundary=decision_bounds,
    # mode="ei",
    # )
    # df_unsmooth["condition"] = "voronoi"
    # df_unsmooth["smooth"] = 0

    # df_trial = pd.concat(
    # [df_voronoi, dfs_test, df_random, df_unsmooth], ignore_index=True
    # )
    # df_trial["trial"] = i

    # df_trials.append(df_trial)

    # out_no_kmeans = pd.concat(df_trials, ignore_index=True)
    out = df_kmeans

    out["human_response"] = R.Response[out.pair_idx.values]
    out["human_rt"] = R.reactionTime[out.pair_idx.values]
    out["k"] = R.kSeg
    out["subject"] = R.fileinfo["subject"]
    out["cat"] = R.fileinfo["cat"]
    out["img"] = R.fileinfo["img"]

    return out


def df_from_file(
    R, decision_bounds=None, n_trials=10, n_pseudocoords=10, use_pointwise_rt=None
):

    R.get_np_coords()
    points = R.points
    pairs = R.get_tested_pairs(all_pairs=False)

    if decision_bounds is not None:
        pass
    else:
        decision_bounds = {"collapsing": [9, 0.1]}

    df_trials = []
    for i in range(n_trials):
        print("\t Creating prior - Trial {}".format(i))
        # create priors
        voronoi_prior = utils.generate_random_voronoi_prior(
            *R.image.shape[:2], R.kSeg, end_iter=0.03
        )
        random_prior = utils.generate_random_prior(*R.image.shape[:2], R.kSeg)

        # create model objects
        Voronoi = SM((0, R.image), mode="array")
        Unsmooth = SM((1, R.image), mode="array")
        model = "c"
        k = R.kSeg
        print("\t\t Fitting Voronoi model (smooth)")
        Voronoi.fit_model(
            model="c",
            n_components=np.array([R.kSeg]),
            layer_stop=1,
            keep=True,
            init=voronoi_prior,
            spatial_smoothing=1,
            init_eps=0.05,
        )
        print("\t\t Fitting Voronoi model (unsmooth)")
        Unsmooth.fit_model(
            model="c",
            n_components=np.array([R.kSeg]),
            layer_stop=1,
            keep=True,
            init=voronoi_prior,
            spatial_smoothing=0,
            init_eps=0.05,
        )

        print("\t\t\t  ...using collapsing bounds")

        df_voronoi = Voronoi.get_decision_rts(
            points,
            pairs,
            R.testedPairs,
            use_pointwise_rts=None,
            use_pseudocoords=n_pseudocoords,
            boundary=decision_bounds,
        )
        df_voronoi["condition"] = "voronoi"

        df_unsmooth = Unsmooth.get_decision_rts(
            points,
            pairs,
            R.testedPairs,
            use_pointwise_rts=None,
            use_pseudocoords=n_pseudocoords,
            boundary=decision_bounds,
        )
        df_unsmooth["condition"] = "unsmooth"

        # print("\t\t\t  ...using constant bounds")
        # const_test = [2, 1, 0.5]

        # dfs_test = pd.concat(
        # [Voronoi.reapply_bounds({"const": [b, -b]}, key=b) for b in const_test],
        # ignore_index=True,
        # )

        # dfs_test_unsmooth = pd.concat(
        # [Unsmooth.reapply_bounds({"const": [b, -b]}, key=b) for b in const_test],
        # ignore_index=True,
        # )

        df_trial = pd.concat([df_voronoi, df_unsmooth], ignore_index=True)
        df_trial["trial"] = i

        df_trials.append(df_trial)

    out = pd.concat(df_trials, ignore_index=True)
    print(" \t Parsing human responses ...")
    R.fit()
    out["human_response"] = R.Response[out.pair_idx.values]
    out["human_psame"] = R.human_psame[out.pair_idx.values]
    out["human_seg_flag"] = R.human_seg_flag[out.pair_idx.values]
    out["human_rt"] = R.reactionTime[out.pair_idx.values]
    out["k"] = R.kSeg
    out["subject"] = R.fileinfo["subject"]
    out["cat"] = R.fileinfo["cat"]
    out["img"] = R.fileinfo["img"]

    return out


# def _df_from_file(
#     R,
#     decision_bounds=(-0.69, 0.69),
#     use_rt=True,
#     voronoi_prior=None,
#     random_prior=None,
#     evidence="first",
#     spatial_smoothing=1,
#     trial_idx=0,
# ):
#     """
#     Generates a DataFrame from a Response class

#     Parameters:
#     -----------
#     R : Response object
#         created from a experiment response file
#     decision_bounds : tup or tup-like
#         used to calculated model reaction time
#     use_rt : bool
#         if True, get human reaction times from Response class
#     voronoi_prior : np.ndarray
#         must be the same shape as Response.image, used in em initial guess
#     random_prior : np.ndarray
#         must be the same shape as Response.image, used in em initial guess
#     evidence : str
#         The method of calculating model reaction time, see
#         analysis.get_model_reaction_time()
#     spatial_smoothing : int
#         1 : spatial smoothing is active 0 : spatial smoothing is not used,
#             posterior probability for a regular mixture model is used instead
#         -1 : posterior class probability is used per pixel
#     trial_idx : int
#         Because the Voronoi prior and random prior are generated using RNG,
#         multiple trials with multiple initializations can be generated. However,
#         Kmeans initialization doesn't depend on RNG. Therefore KMeans
#         initialization only runs if trial_idx==0

#     Returns:
#     --------
#     df : pd.DataFrame
#         df is of type generated by analysis.get_df()
#     """
#     if voronoi_prior is None:
#         voronoi_prior = utils.generate_random_voronoi_prior(
#             *R.image.shape[:2], R.kSeg, end_iter=0.05
#         )
#     if random_prior is None:
#         random_prior = utils.generate_random_prior(*R.image.shape[:2], R.kSeg)

#     Voronoi = SM((0, R.image), mode="array")
#     Random = SM((1, R.image), mode="array")
#     model = "c"
#     k = R.kSeg

#     if spatial_smoothing == 1:
#         print("\tSpatial smoothing ON")
#         print("\t\tSegmenting Voronoi... ")
#         Voronoi.fit_model(
#             model=model,
#             n_components=np.array([k]),
#             layer_stop=1,
#             keep=True,
#             init=voronoi_prior,
#             init_eps=0.05,
#         )
#         print("\t\tSegmenting Random... ")
#         Random.fit_model(
#             model=model,
#             n_components=np.array([k]),
#             layer_stop=1,
#             keep=True,
#             init=random_prior,
#             init_eps=0.1,
#         )
#     else:
#         print("\tSpatial smoothing OFF")
#         param = spatial_smoothing
#         print("\t\tSegmenting Voronoi... ")
#         Voronoi.fit_model(
#             model=model,
#             n_components=np.array([k]),
#             layer_stop=1,
#             keep=True,
#             init=voronoi_prior,
#             init_eps=0.05,
#             spatial_smoothing=param,
#         )
#         print("\t\tSegmenting Random... ")
#         Random.fit_model(
#             model=model,
#             n_components=np.array([k]),
#             layer_stop=1,
#             keep=True,
#             init=random_prior,
#             init_eps=0.1,
#             spatial_smoothing=param,
#         )
#     # extract probability maps
#     if use_rt:
#         pairs = R.get_tested_pairs(all_pairs=False)
#         rts = R.reactionTime
#         responses = R.Response
#         p_same = None
#     else:
#         pairs = R.get_tested_pairs()
#         rts = None
#         responses = None
#         p_same = None
#     df = pd.concat(
#         [
#             analysis.get_df(
#                 pairs,
#                 pmaps["Voronoi"],
#                 decision_bounds=(decision_bounds[0], decision_bounds[1]),
#                 reaction_times=rts,
#                 responses=responses,
#                 p_same=p_same,
#                 condition="voronoi",
#                 evidence=evidence,
#             ),
#             analysis.get_df(
#                 pairs,
#                 pmaps["Random"],
#                 decision_bounds=(decision_bounds[0], decision_bounds[1]),
#                 reaction_times=rts,
#                 responses=responses,
#                 p_same=p_same,
#                 condition="random",
#                 evidence=evidence,
#             ),
#         df = pd.concat(
#             [
#                 analysis.get_df(
#                     pairs,
#                     pmaps["Voronoi"],
#                     decision_bounds=(decision_bounds[0], decision_bounds[1]),
#                     reaction_times=rts,
#                     responses=responses,
#                     p_same=p_same,
#                     condition="voronoi",
#                     evidence=evidence,
#                 ),
#                 analysis.get_df(
#                     pairs,
#                     pmaps["Random"],
#                     decision_bounds=(decision_bounds[0], decision_bounds[1]),
#                     reaction_times=rts,
#                     responses=responses,
#                     p_same=p_same,
#                     condition="random",
#                     evidence=evidence,
#                 ),
#             ]
#         )

#     df["k"] = k
#     for item in list(R.fileinfo.keys())[1:-1]:
#         df[item] = R.fileinfo[item]

#     return df


# def df_from_file(
#     file,
#     decision_bounds=(-0.69, 0.69),
#     use_rt=True,
#     use_KL=False,
#     trial_idx=0,
#     evidence="first",
# ):
#     """
#     Wrapper around _df_from_file that runs fits for Voronoi initialization,
#     random initialization, and KMeans initialization with both spatial smoothing
#     on and spatial smoothing off

#     Parameters:
#     -----------
#     file : str
#         filepath to experiment file
#     decision_bounds : tup or tup-like
#         used to calculated model reaction time
#     use_rt : bool
#         if True, get human reaction times from Response class
#     evidence : str
#         The method of calculating model reaction time, see
#         analysis.get_model_reaction_time()
#     trial_idx : int
#         Because the Voronoi prior and random prior are generated using RNG,
#         multiple trials with multiple initializations can be generated. However,
#         Kmeans initialization doesn't depend on RNG. Therefore KMeans
#         initialization only runs if trial_idx==0

#     Returns:
#     --------
#     df : pd.DataFrame
#         df is of type generated by analysis.get_df()
#     """
#     print("Importing file: {}".format(file))
#     R = Res(file)
#     # create random initial Voronoi/Random priors
#     print("\tGenerating voronoi prior...")
#     vd = utils.generate_random_voronoi_prior(*R.image.shape[:2], R.kSeg, end_iter=0.05)
#     print("\tGenerating random prior...")
#     rand = utils.generate_random_prior(*R.image.shape[:2], R.kSeg)
#     dfs = []
#     for spatial_smoothing in [1, 0, -1]:
#         df = _df_from_file(
#             R,
#             decision_bounds=decision_bounds,
#             use_rt=use_rt,
#             trial_idx=trial_idx,
#             spatial_smoothing=spatial_smoothing,
#             voronoi_prior=vd,
#             random_prior=rand,
#             evidence=evidence,
#         )
#         df["smoothness"] = spatial_smoothing
#         dfs.append(df)

#     df_out = pd.concat(dfs, axis=0, ignore_index=True)

#     return df_out


# def run_trials(file, n_trials, evidence="first"):
#     """
#     Wrapper around df_from_file that runs that function multiple times

#     Parameters:
#     -----------
#     file : str
#         filepath to experiment file
#     n_trials : int
#         The total number of trials to run, or, the total number of Voronoi and
#         random priors generated using RNG per image

#     Returns:
#     --------
#     df : pd.DataFrame
#         df is of type generated by analysis.get_df()
#     """
#     dfs = []
#     for i in range(n_trials):
#         print("File: {} | Trial {}".format(file, i))
#         print("\t")
#         df = df_from_file(file, trial_idx=i, evidence=evidence)
#         df["trial"] = i
#         dfs.append(df)
#         print("\t")
#     out = pd.concat(dfs, axis=0, ignore_index=True)

#     return out


# def run_all_files(files, n_trials=10, evidence="first"):
#     """
#     Wrapper around run_trials that runs that function for every file

#     Parameters:
#     -----------
#     files : list of str
#         list of filenames as str for input into run_trials
#     n_trials : int
#         The total number of trials per file
#     Returns:
#     --------
#     df : pd.DataFrame
#         df is of type generated by analysis.get_df()
#     """
#     out = pd.concat(
#         [run_trials(file, n_trials, evidence=evidence) for file in files],
#         axis=0,
#         ignore_index=True,
#     )

#     return out


# def run_n_trials(
#     Response,
#     layers=range(1),
#     n_trials=10,
#     type="voronoi",
#     smooth=1,
#     n_pseudocoords=10,
#     use_pointwise_rts=True,
# ):

#     max_layer = max(layers)

#     R = Response

#     _points = np.concatenate(
#         [R.xGrid[..., np.newaxis], R.yGrid[..., np.newaxis]], axis=1
#     )
#     # default transformation is to numpy coords
#     points = [tb.transform_coord_system(coord) for coord in _points]

#     # all_pairs = False ensures that only pairs with a reaction time are considered
#     pairs = R.get_tested_pairs(all_pairs=False)
#     obj = SM((0, R.image), mode="array")

#     pairwise_distances = []
#     decision_rts = []
#     dynamic_maps = []
#     dynamics_dfs = []

#     for trial in range(n_trials):
#         print(trial, "...")
#         if type == "voronoi":
#             _prior = utils.generate_random_voronoi_prior(
#                 *R.image.shape[:2], R.kSeg, end_iter=0.05
#             )
#             _eps = 0.05
#         else:
#             _prior = utils.generate_random_prior(*R.image.shape[:2], R.kSeg)
#             _eps = 0.1

#         obj.fit_model(
#             model="c",
#             n_components=np.array([R.kSeg]),
#             layer_stop=max_layer,
#             keep=True,
#             init=_prior,
#             init_eps=_eps,
#             spatial_smoothing=smooth,
#         )

#         # when running multiple trials set kernel_size = None
#         # instead we will use a larger kernel over ALL trials
#         for layer in layers:
#             obj.parse_layer(layer)
#             obj.get_decision_rts(
#                 points,
#                 pairs,
#                 R.testedPairs,
#                 kernel_size=None,
#                 use_pointwise_rts=use_pointwise_rts,
#                 use_pseudocoords=n_pseudocoords,
#             )
#             df = obj.dynamics_df_pairwise
#             df["layer"] = layer

#             pair_idxs = df.pair_idx.values
#             df["human_rt"] = R.reactionTime[pair_idxs]
#             df["human_decision"] = R.Response[pair_idxs]
#             df["trial"] = trial

#             dynamics_dfs.append(df)

#             dynamic_maps.append(obj.pointwise_rts)
#             pairwise_distances.append(obj.pairwise_distances)
#             decision_rts.append(obj.decision_rts)

#     df = pd.concat(dynamics_dfs, axis=0, ignore_index=True)

#     res = {"df": df, "dynamic_maps": dynamic_maps}

#     ## number of yes/no observations per trial
#     # trial_n_obs = {key: np.zeros(len(pairwise_distances)) for key in ["y", "n"]}

#     # for i in range(len(pairwise_distances)):
#     # for key in ["y", "n"]:
#     # n_obs = len(pairwise_distances[i][key + "s"])
#     # trial_n_obs[key][i] = n_obs

#     # max_obs = {key: np.max(trial_n_obs[key]).astype("int") for key in ["y", "n"]}

#     # padded_distances = {
#     # key: np.zeros((len(pairwise_distances), max_obs[key])) for key in ["y", "n"]
#     # }

#     # padded_rts = {
#     # key: np.zeros((len(pairwise_distances), max_obs[key])) for key in ["y", "n"]
#     # }

#     # for i in range(len(pairwise_distances)):
#     # for key in ["y", "n"]:
#     # d_arr = pairwise_distances[i][key + "s"]
#     # d_arr_pad = np.pad(d_arr, (0, max_obs[key] - len(d_arr)), mode="edge")
#     # rt_arr = decision_rts[i][key]
#     # rt_arr_pad = np.pad(rt_arr, (0, max_obs[key] - len(rt_arr)), mode="edge")

#     # padded_distances[key][i, :] = d_arr_pad
#     # padded_rts[key][i, :] = rt_arr_pad

#     # res = {
#     # "padded_distances": padded_distances,
#     # "padded_rts": padded_rts,
#     # "dynamic_maps": dynamic_maps,
#     # }

#     return df


#     print("Hello")

#     files = ns(glob("data/*"))

#     df = run_all_files(files, evidence="area")

#     df.to_csv("reaction_time_responses_log_area_model_smooth_test_all_files.csv")

if __name__ == "__main__":
    files = ns(glob("data/data_processing/*"))
    Response_objs = []

    for i, file in enumerate(files[:]):
        print("Processing {}:{}".format(i, file))
        R = Res(file)
        R.fit()
        R.run_dynamics_model(n_trials=10, n_pseudocoords=10)

        R.optimize_params(
            loss_type="mse",
            verbose=True,
            penalize_zeros=True,
            split_by_response=True,
            annealing_step=True,
            mode="all",
            params=["automult", "online_rt", "ei_rt", "wei_rt"],
        )

        R.opt_automult_param2d = np.array(
            [R.opt_params["automult"], R.opt_params["online_rt"]]
        )

        for i in range(2):
            R.optimize_params_2d(init_guess=R.opt_automult_param2d, loss_type="mse")

        filename = R.filename.split("/")[-1].split(".")[0]

        import_utils._pickle(
            R, "response_objs/mse/{}_Response_obj.pkl".format(filename)
        )
